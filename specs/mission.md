...
4. **The demo fits in a pocket — now a bonus, not a requirement** *(added 2026-07-11; revised 2026-07-12 replan)*: **"Talk to My Trip"**, the native iOS companion app (`ios/TalkToMyTrip/`), was **submitted to App Review on 2026-07-11** (the Phase 17 full three-act build — access-gated, AI-consent flow, empty-start onboarding included; runbook `IOS_DEPLOY.md`). Review latency is outside our control, so **App Store approval before event day is no longer a success criterion**: the web surfaces are the demo path no matter what, and if the app is approved in time it joins the demo as a bonus. No new binary is uploaded until the in-review one is approved (a replacement restarts the queue); post-approval fixes ship as v1.0.1+.
