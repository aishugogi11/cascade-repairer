...
#
# PR description: the commit message / title is used as the PR body by default.
# For a richer description, write PR_BODY.md at the repo root (gitignored) and
# this script uses it as the PR description verbatim. A hand-edited PR body on
# GitHub is never clobbered.
#
# Usage:
#   ./git_pull_dev.sh
...
if [ -f "PR_BODY.md" ]; then
  # An optional draft (gitignored) is used as the PR description, verbatim.
  PR_BODY="$(cat PR_BODY.md)"
  BODY_SOURCE="PR_BODY.md"
  echo "==> PR description from PR_BODY.md"
else
  # Default: the commit message / title is the PR description.
  PR_BODY="${COMMIT_MSG}"
...
if [ "${CURRENT_BRANCH}" = "${BASE_BRANCH}" ]; then
  ...
  if gh pr view "${CURRENT_BRANCH}" >/dev/null 2>&1; then
    ...
    if [ "${BODY_SOURCE}" = "PR_BODY.md" ]; then
      ...
      # Never clobber a hand-written body with the default. Only fill in a
      # PR that currently has an empty description.
