...
6. The fixes are reflected on the dashboard through polling, not a push connection. Every 1.5 seconds, [`page.html`](backend/api/assets/cascade/page.html) requests `GET /v1/itinerary/status/{trip_id}`. The endpoint in [`itinerary_ui.py`](backend/api/itinerary_ui.py) reads the trip and item rows from BigQuery, adds the latest booking-derived detail for each item, and includes the current consent state.
   ...
   **Current Phase 32 gap:** the repair booking already contains the replacement flight and the page can show its `why_chosen` sentence and price delta, but `_rebook_flight()` does not yet update the flight's core `itinerary_items` fields. The main flight card therefore reaches `fixed` while still showing the original time and price. The current [`requirements.md`](specs/2026-07-16-repaired-flight-on-page/requirements.md) addresses this at the source of truth: write the replacement timestamps, fare, currency, and flight identity to the item before its final `fixed` transition, then show an old → new treatment on the card. Until that Phase 32 change is implemented and deployed, the status flip is live but the headline replacement details remain stale.
