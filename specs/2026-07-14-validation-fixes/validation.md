...
- **Hermetic suite green with zero Sabre env vars.** All `cert`-marked tests deselected by `pytest.ini` `addopts`; no collected test attempts network access. The two new hermetic tests are collected and pass in this bare run:
  - `tests/test_sabre_cert_cleanup.py::test_shape_drift_after_created_pnr_still_cancels`
...
- **Scope discipline:** `git diff vb/dev -- backend/api/sabre/` is empty; no changes under `specs/` outside `specs/2026-07-13-sabre-cert-exploration/` (probes + notes) and this spec directory; no raw credential or token material in the diff (`git diff vb/dev...HEAD` scan).
...
5. **Deferred, tracked, not blocking this branch:** the close-out re-validation session against Phase 25 + 26 criteria, and the different-day dated `pytest -m cert` artifact appended to `sabre-cert-notes.md` (event-day morning at the latest, D4). Phase 26 is marked `[x] COMPLETE` in the roadmap only when the re-validation passes.
