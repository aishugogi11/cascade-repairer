...
- **Six-case metro-alias coverage** (criterion 6, literal reading): a parametrized test over `NYC→JFK`, `WAS→IAD`, `CHI→ORD` in **both** request positions, asserting the aliased pair reaches the market check and the captured client request.
- **POST-path token-refresh coverage** (criterion 5 note): mocked-transport test that `_post` retries a 401 exactly once with a freshly minted bearer and raises on a second 401 (the committed tests exercise `_send_with_refresh` through GET only).
- **Clear stale options on empty/failed searches** (report § Risks): `search_flights_impl`'s no-options, error, and unsupported-market returns leave the previous search's options in `_SESSION_FLIGHT_OPTIONS`/`_LATEST_SEARCH`, so a traveler can book a stale choice after being told there are no flights — clear both on every non-optioned return. (Phase 22's `_LATEST_SEARCH` expiry hardening is unchanged and complementary.)
- **`git_pull_dev.sh` base-branch pin** (report § Risks): default `BASE_BRANCH` to `vb/dev` ahead of the `gh`/`origin/HEAD` fallbacks — today it resolves to `main` and `--admin` merges immediately.
