...
4. **The demo fits in a pocket — now a bonus, not a requirement** *(added 2026-07-11; revised 2026-07-16 after App Review)*: **"Talk to My Trip"**, the native iOS companion app (`ios/TalkToMyTrip/`), was submitted to App Review on 2026-07-11 (the Phase 17 full three-act build — access-gated, AI-consent flow, empty-start onboarding included; runbook `IOS_DEPLOY.md`). Apple rejected v1.0 (3) on 2026-07-16 under Guideline 3.2 because public distribution did not match its limited hackathon-event audience. The recovery path is **Unlisted App Distribution**: resubmit the same final build, keep the App Store Connect distribution method Public as Apple's request flow requires, and file the unlisted-app request so the event audience installs through a direct link while the access code continues to prevent unauthorized use. Review latency remains outside our control, so **an App Store link before event day is not a success criterion**: the web surfaces and direct Xcode/TestFlight installs remain the guaranteed demo paths, and an unlisted approval in time joins them as a bonus. No replacement binary is uploaded during the unlisted recovery; later fixes ship as an update only after v1.0 (3)'s outcome is settled.
...
- A general-purpose travel product beyond the hackathon's scope — the App Store app ("Talk to My Trip") is a finished limited-audience special-event app distributed unlisted, not a public consumer-product pivot.
