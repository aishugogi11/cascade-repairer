...
4. **The demo fits in a pocket** *(added 2026-07-11; revised 2026-07-11 replan #2)*: **"Talk to My Trip"**, a native iOS companion app (`ios/TalkToMyTrip/`), is approved on the App Store before event day — voice-first trip booking and the break→heal cascade running on a real iPhone against the deployed backend. The MVP-first submission plan collapsed into one: the Phase 16 build was never submitted, so the **Phase 17 full three-act build (access-gated, with the AI-consent flow App Review now requires) is the first submission**, guided by `IOS_DEPLOY.md` at the repo root.
